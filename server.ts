...
// Tuiles carte via le même serveur (évite blocage navigateur / CSP en production)
app.get("/tiles/osm/:z/:x/:y.png", async (req, res) => {
  const { z, x, y } = req.params;
  const tileUrl = `https://tile.openstreetmap.org/${z}/${x}/${y}.png`;
  try {
    const response = await fetch(tileUrl, {
      headers: { "User-Agent": "ASSOCIE.AI/2.0 (property map; contact@associe.ai)" },
    });
    if (!response.ok) {
      return res.status(response.status).send("Tile unavailable");
    }
...
function isGoogleMapsLink(raw: string): boolean {
  const url = (raw || "").trim();
  if (!url) return false;
  return /maps\.app\.goo\.gl|google\.com\/maps|maps\.google\.com|goo\.gl\/maps/i.test(url);
}

function normalizePropertyImageUrl(raw: string): string {
  let url = (raw || "").trim();
  if (!url) return "";
...
app.post("/api/properties", requireAuth, async (req, res) => {
  ...
  const normalizedImageUrl = normalizePropertyImageUrl(imageUrl || "");
  ...
  try {
    const result = await queryOne(
      ...
      [tenantId, projectId, name, address || '', latitude, longitude, price || 0, surface || 0, description || '', normalizedImageUrl]
    );
    res.json(result);
  } catch (err: any) {
...
// Extraire lat/lng depuis un lien Google Maps (ex. maps.app.goo.gl)
app.post("/api/resolve-maps-link", requireAuth, async (req, res) => {
  const { url } = req.body;
  if (!url || typeof url !== "string") {
    return res.status(400).json({ error: "URL Google Maps manquante." });
  }
  if (!isGoogleMapsLink(url)) {
    return res.status(400).json({ error: "Ce lien n'est pas un lien Google Maps reconnu." });
  }
  ...
  try {
    const response = await fetch(fetchUrl, {
      redirect: "follow",
      headers: { "User-Agent": "ASSOCIE.AI/2.0 (maps link resolver)" },
    });
    const finalUrl = response.url;
    const coords = parseCoordsFromGoogleMapsUrl(finalUrl);
    if (!coords) {
      return res.status(422).json({
        error:
          "Impossible d'extraire les coordonnées de ce lien. Copiez l'adresse dans le champ « Adresse » et cliquez sur Géocoder.",
    ...
    return res.json({
      ...
      resolvedUrl: finalUrl,
      message: "Position extraite du lien Google Maps.",
    });
  } catch (err: any) {
    console.error("resolve-maps-link failed:", err);
    return res.status(502).json({ error: err.message || "Échec de lecture du lien Google Maps" });
  }
});

// Photo du bien (proxy — pour affichage dans <img> popup carte avec token en query)
app.get("/api/properties/:id/photo", requireAuth, async (req, res) => {
  const user = (req as any).user;
  const tenantId = user.tenantId;
  ...
  try {
    const row = await queryOne(
      `SELECT image_url FROM properties WHERE id = $1 AND tenant_id = $2`,
      [propertyId, tenantId]
    );
    if (!row?.image_url) {
    ...
    const imageUrl = normalizePropertyImageUrl(row.image_url);
    const response = await fetch(imageUrl, {
      headers: {
        "User-Agent": "ASSOCIE.AI/2.0 (property photo proxy)",
        Accept: "image/*,*/*",
      },
      redirect: "follow",
    });
    ...
    if (!response.ok) {
      return res.status(502).json({
        error: "Impossible de charger l'image à cette URL. Utilisez un lien direct (.jpg, .png) ou Google Drive en mode partage.",
      });
    }
    ...
    if (!contentType.startsWith("image/")) {
      return res.status(400).json({
        error: "L'URL ne pointe pas vers une image directe. Collez le lien de la photo elle-même (finissant par .jpg ou .png).",
      });
    }

    const buf = Buffer.from(await response.arrayBuffer());
    if (buf.length > 8 * 1024 * 1024) {
      return res.status(413).send("Image trop volumineuse (max 8 Mo)");
    }
    ...
    console.error("Property photo proxy failed:", err);
    return res.status(502).json({ error: err.message || "Échec du chargement de l'image" });
  }
});
